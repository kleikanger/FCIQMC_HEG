#ifndef HAMILTONIAN3DHEG_CPP
#define HAMILTONIAN3DHEGT_CPP
...
//TODO Optimization save mom as Nx3 vector,

void hamiltonian3DHEG::init(
		int i_numpart,
		int i_norb,
	    imputVars* vars,
		int* pi_k,
...
{
	d_l = pow( 4./3.*PI*vars->iGETnumpart()*pow(vars->dGETrs(), 3), 1./3.); //3D
	d_k1 = 2.*PI*PI/(d_l*d_l);
	d_k2 = 4.*PI/(d_l*d_l*d_l); // 3D
...
double hamiltonian3DHEG::getDiagonalElement(
...
double hamiltonian3DHEG::getSingleExcitationAmplitude (
		const int i_sk,
		const int i_sp,
		const int *pi_s) const
{
...
double hamiltonian3DHEG::getDoubleExcitationAmplitude(
			const int iak, 
			const int ial,
			const int iap,
			const int iaq) const
{
	...
	if ((pi_l[iak/2]+pi_l[ial/2])!=(pi_l[iap/2]+pi_l[iaq/2]))
		cout << "\nerr\n";
	if ((pi_m[iak/2]+pi_m[ial/2])!(pi_m[iap/2]+pi_m[iaq/2]))
		cout << "\nerr\n";
	*/
	return ret*d_k2;
}
